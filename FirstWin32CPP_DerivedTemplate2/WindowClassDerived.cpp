...
//включаемые файлы
#include "WindowClassDerived.h"						//описание класса окна Windows (переопределЄнный)

//класс WindowClass

//инициализаци€ статических данных
unsigned short int WindowClassDerived::usiWndNum = 0;		//количество объектов переопределЄнного класса
...
WindowClassDerived::WindowClassDerived(WNDCLASSEX& wc, const TCHAR *szWndTitle) : WindowClassTemplate(wc, szWndTitle)
{
	usiWndNum++;		//увеличиваем количество объектов данного класса
}

WindowClassDerived::WindowClassDerived(WindowClassDerived& wcObj) : WindowClassTemplate(wcObj)
{
	usiWndNum++;		//увеличиваем количество объектов данного класса
}
...
{
	if(hWnd)
		this->OnClose(hWnd);		//закрываем окно, использу€ механизм виртуальных функций
}
...
void WindowClassDerived::OnPaint(HWND hWnd)
{
	...
	DrawText(hDC, TEXT("ѕереопределЄнный класс (единичное наследование)."), -1, &rect, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
