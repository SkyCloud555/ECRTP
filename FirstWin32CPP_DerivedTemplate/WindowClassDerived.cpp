...
WindowClassDerived::WindowClassDerived(HINSTANCE hInstance, const TCHAR *szClassName, const TCHAR *szWndTitle) : WindowClass<WindowClassDerived>(hInstance, szClassName, szWndTitle)
{
	usiWndNum++;		//увеличиваем количество объектов данного класса
}

WindowClassDerived::WindowClassDerived(WNDCLASSEX& wc, const TCHAR *szWndTitle) : WindowClass<WindowClassDerived>(wc, szWndTitle)
{
	usiWndNum++;		//увеличиваем количество объектов данного класса
}
...
void WindowClassDerived::OnPaint(HWND hWnd)
{
	...
	DrawText(hDC, TEXT("Переопределённый класс С++ с шаблоном для окна Windows посредством Win32 API."), -1, &rect, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
